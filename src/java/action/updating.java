...
public class updating extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            keyGeneration.init(128);
            SecretKey secretKey = keyGeneration.generateKey();
            System.out.println("secret key:" + secretKey);

            encryption e = new encryption();
            String cipherText = e.encrypt(fileContent, secretKey);

            byte[] byteCode = secretKey.getEncoded();//encoding secretkey
            String secureKey = Base64.encode(byteCode);
            ...
            st.executeUpdate("insert into updates (filename, username, time)values('" + fileName + "','" + userName + "','" + time + "')");
//             
            int i = st.executeUpdate("update files set content='" + cipherText + "',file_key='" + secureKey + "' where idfiles='" + fileId + "'");
            if (i != 0) {
